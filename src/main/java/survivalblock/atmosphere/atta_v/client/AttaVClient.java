...
public class AttaVClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		WorldRenderEvents.AFTER_ENTITIES.register(worldRenderContext -> {
			...
			worldPathComponent.getPaths().forEach(entityPath -> {
				renderPath(entityPath, matrixStack, lines);
			});
			/*
			worldPathComponent.getPaths().forEach(entityPath -> {
				drawFilledPolygon(entityPath.nodes, matrixStack, vertexConsumerProvider);
			});
			 */
			matrixStack.pop();
		});
	...
	private void renderPath(EntityPath entityPath, MatrixStack matrixStack, VertexConsumer lines) {
		...
		for (int i = 1; i < size + 1; i++) {
			...
			WandererRenderer.drawLine(previous, current, matrixStack, lines, color);
			previous = current;
		}
	}
	...
	/*
	// doesn't work nooooo
	public static void drawFilledPolygon(List<Vec3d> vec3ds, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider) {
		VertexConsumer vertices = vertexConsumerProvider.getBuffer(RenderLayer.getEndPortal());
		if (vec3ds.size() <= 2) {
			return;
		}
		matrixStack.push();
		Matrix4f model = matrixStack.peek().getPositionMatrix();
		List<Vector3f> points = vec3ds.stream().map(Vec3d::toVector3f).toList();
		for (var three1 : points) {
			for (var three2 : points) {
				if (three1 == three2) {
					continue;
				}
				vertices.vertex(model, three1.x, three1.y, three1.z);
				vertices.vertex(model, three2.x, three2.y, three2.z);
			}
		}
		matrixStack.pop();
	}
